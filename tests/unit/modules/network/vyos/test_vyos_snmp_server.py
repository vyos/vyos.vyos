...
class TestVyosSnmpServerModule(TestVyosModule):
            ...
            dict(
                config=dict(
                    ...
                    snmp_v3=dict(
                        engine_id="34",
                        groups=[
                            dict(group="default", mode="rw", seclevel="priv", view="view1")
                        ],
                        trap_targets=[
                            dict(
                                address="20.12.1.1",
    ...
    def test_snmp_server_replaced(self):
        ...
        commands = [
            ...
            'set service snmp v3 group default mode rw', 'set service snmp v3 group default seclevel priv', 'set service snmp v3 group default view view1',
    ...
    def test_snmp_server_parsed(self):
        ...
        parsed_list = {
            'communities': [
                {'authorization_type': 'rw', 'clients': ['1.1.1.0/24', '12.1.1.0/24'], 'name': 'routers'},
                {'authorization_type': 'ro', 'name': 'switches'}
            ],
            'contact': 'admin@example.com',
            'description': 'snmp_config', 'smux_peer': 'peer1',
            'snmp_v3': {
                'engine_id': '34',
                'groups': [
                    {'group': 'default', 'mode': 'rw', 'seclevel': 'priv', 'view': 'view1'}
                ]
            },
            'trap_source': '1.1.1.1', 'trap_target': {'address': '10.10.1.1'}
        }
    ...
    def test_snmp_server_gathered(self):
        ...
        gathered_list = {
            ...
            'snmp_v3': {
                'users': [
                    {
                        'authentication': {'plaintext_key': 'abc1234567', 'type': 'sha'},
                        'privacy': {'plaintext_key': 'abc1234567', 'type': 'aes'},
                        'user': 'admin_user'
                    },
                    {
                        'authentication': {'plaintext_key': 'opq1234567', 'type': 'sha'},
                        'privacy': {'plaintext_key': 'opq1234567', 'type': 'aes'},
                        'user': 'guest_user'
                    }
                ]
            }
